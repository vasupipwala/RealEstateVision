...
def validation_summary(db: sqlite3.Connection = Depends(get_db)):
    """
    Retrieve the summary of image quality and dataset validation outcomes.
    """
    row = db.execute(
        """
        SELECT
...
def get_validation_record(image_id: str, db: sqlite3.Connection = Depends(get_db)):
    """
    Retrieve the validation record for a specific image by its `image_id`.
    
    Expected `image_id` format:
    `{split}_{roomtype}_{id}`
    
    Example:
    `train_bedroom_000001`
    
    Where:
    - `split` is the dataset split, such as `train` or `test`
    - `roomtype` is the room category, such as `bathroom`, `bedroom`, `diningroom`,     `kitchen`, or `livingroom`
    - `id` is a zero-padded numeric identifier, such as `000001` or `000002`
    """
...
def list_validation_records(
    ...
    """
    Group search the validation record.
    """
    conditions = []
    params = []
...
def list_dataset_versions(db: sqlite3.Connection = Depends(get_db)):
    """
    Retrieve all the dataset-versions available in the database.
    """
    rows = db.execute(
        """
        SELECT
