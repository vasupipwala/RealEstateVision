...
def compute_class_thresholds(results_df):
    train_df = results_df[
    ...
    for class_name, group in train_df.groupby("class_name"):
        if len(group) > 0:
            thresholds[class_name] = float(np.percentile(group["tenengrad_score"], TENENGRAD_PERCENTILE))
        else:
            thresholds[class_name] = 0.0

    global_fallback = float(np.percentile(train_df["tenengrad_score"], TENENGRAD_PERCENTILE)) if len(train_df) > 0 else 0.0
    return thresholds, global_fallback
...
def main():
    ...
    class_thresholds, global_fallback_threshold = compute_class_thresholds(results_df)

    for idx, row in results_df.iterrows():
        threshold = class_thresholds.get(row["class_name"], global_fallback_threshold)
        results_df.at[idx, "tenengrad_threshold_used"] = threshold

        is_blurry = (
            row["is_corrupt"] == 0 and
            pd.notna(row["tenengrad_score"]) and
            row["tenengrad_score"] <= threshold
        )
        results_df.at[idx, "is_blurry"] = int(is_blurry)
    ...
    for group_indices in duplicate_groups:
        ...
        has_test = (group_df["split"] == "test").any()
        ...
        for idx in group_indices:
            ...
            elif cross_class and same_split and row["split"] == "train":
                results_df.at[idx, "is_duplicate"] = 0
                results_df.at[idx, "drop_reason"] = "kept_train_cross_class_duplicate"

            elif cross_class and same_split and row["split"] == "test":
                results_df.at[idx, "is_duplicate"] = 1
                results_df.at[idx, "drop_reason"] = "drop_test_cross_class_duplicate"
    ...
    duplicate_pairs = export_df[(
        export_df["is_duplicate"] == 1
    ) | (
        export_df["drop_reason"].isin(["kept_train_cross_class_duplicate"])
    )][[
    ...
    threshold_report = pd.DataFrame([
        {"class_name": class_name, "tenengrad_threshold": threshold}
        for class_name, threshold in sorted(class_thresholds.items())
    ])
    threshold_report.loc[len(threshold_report)] = ["__global_fallback__", global_fallback_threshold]
    threshold_report.to_csv(REPORT_BASE / "tenengrad_class_thresholds.csv", index=False)
    ...
    print(f"Per-class Tenengrad percentile: {TENENGRAD_PERCENTILE}")
    print(f"Threshold report: {REPORT_BASE / 'tenengrad_class_thresholds.csv'}")
